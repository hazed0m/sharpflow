  ...
  return (
    <section className="rounded-[2rem] border border-slate-200/20 bg-white/95 p-6 shadow-glow ring-1 ring-slate-200/30 transition-colors duration-300 dark:border-white/10 dark:bg-slate-950/90 dark:ring-white/10">
      <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Focus metrics</p>
          <h2 className="mt-2 text-2xl font-semibold text-white">Low-key progress overview</h2>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4 text-sm text-slate-200">
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Completed</p>
          <p className="mt-3 text-3xl font-semibold text-emerald-300">{metrics.completedCount}</p>
        </div>
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4 text-sm text-slate-200">
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Burned</p>
          <p className="mt-3 text-3xl font-semibold text-amber-300">{metrics.burnedCount}</p>
        </div>
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4 text-sm text-slate-200">
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Memories</p>
          <p className="mt-3 text-3xl font-semibold text-sky-300">{metrics.memoryCount}</p>
        </div>
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-4 text-sm text-slate-200">
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Avg focus</p>
          <p className="mt-3 text-3xl font-semibold text-white">{metrics.averageFocusMinutes} min</p>
        </div>
      </div>
    </section>
  )
